...
public:
    ...
    void roll_ndarray(py::array_t<D> arr, size_t axis, size_t window, size_t min_periods) {
        /* inplace rolling, accepts one pointer. */
        py::buffer_info info_arr = arr.request();
        D* ptr_arr = static_cast<D*>(info_arr.ptr);
        std::vector<py::ssize_t> shape = info_arr.shape;
        std::vector<py::ssize_t> strides = info_arr.strides;
        size_t size_arr = info_arr.size;
        size_t ndim = info_arr.ndim;
        assert(ndim > 0);
        for (int i = 0; i != ndim; ++i){
            strides[i] /= info_arr.itemsize;
        }
...
void declare_array_RollingStatistics(py::module& m, const std::string& typestr) {
     ...
     * Code is referenced from https://stackoverflow.com/questions/47487888/pybind11-template-class-of-many-types
     */
    ...
    py::class_<Class>(m, pyclass_name.c_str())
        ...
        .def("push", &Class::push, py::arg("value"))
        .def("pop", &Class::pop)
        .def("compute", &Class::compute)
        .def("roll_ndarray", &Class::roll_ndarray, py::arg("arr"), py::arg("axis"), py::arg("window"), py::arg("min_periods"));
}
